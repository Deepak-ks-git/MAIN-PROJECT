...
class _ProcurementDetailsPageState extends State<ProcurementDetailsPage> {
  ...
  Future<void> fetchProcurements() async {
    final response = await http.get(Uri.parse('http://192.168.0.102:3000/getProc?procId=${widget.procId}'));
  ...
  Future<void> fetchActiveItems() async {
    final response = await http.get(Uri.parse('http://192.168.0.102:3000/getActiveItems'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      setState(() {
  ...
  Future<void> fetchProcurementDetails(String procId) async {
    final response = await http.get(Uri.parse('http://192.168.0.102:3000/ProcureItemDetails?procId=$procId'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      setState(() {
  ...
  void addItemToProcurement() async {
    if (selectedItem.isNotEmpty && quantity > 0) {
      final url = Uri.parse('http://192.168.0.102:3000/addItemToProcurement');
  ...
  void deleteItemFromProcurement(String itemId) async {
    final url = Uri.parse('http://192.168.0.102:3000/deleteItemFromProcurement');
    final response = await http.post(url, body: {
      'procId': widget.procId,
      'itemId': itemId,
    });
  ...
  void startProcurement() async {
    if (procureItemDetails.isNotEmpty) {
      ...
      final url = Uri.parse('http://192.168.0.102:3000/startProc');
