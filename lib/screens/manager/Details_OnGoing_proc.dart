...
class _ProcurementDetailsPageState extends State<Details_OnGoing_proc> {
  ...
  Future<void> fetchPurchaseRequestDetails(String procId) async {
    final response = await http.get(Uri.parse(
        'http://192.168.0.102:3000/reqdetails_for_sending?procId=$procId'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      setState(() {
  ...
  Future<void> fetchPurchaseRequestStatus() async {
    final response = await http.get(
        Uri.parse('http://192.168.0.102:3000/purchase_request_status?procId=${widget.procId}'));
  ...
  Future<void> cancelPurchaseRequest() async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.0.102:3000/cancel_request'),
