...
class PLACE_ORDER extends StatefulWidget {
  ...
  const PLACE_ORDER({Key? key, required this.procId, required this.purchaseReqId})
      : super(key: key);
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              if (deliveryDate.isNotEmpty) {
                                ...
                                // Handle case where delivery date is empty
                                print('Delivery date is empty');
                              }
                            },
  ...
  Future<void> updateOrderStatus() async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.1.143:3000/UPDATE_ORDER_STATUS'),
      ...
      if (response.statusCode == 200) {
        ...
        // Optionally update UI after order status update
      } else {
