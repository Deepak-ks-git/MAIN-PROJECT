...
import 'package:project3/screens/manager/VerifyOrdersDetails.dart';

class VerifyOrders extends StatefulWidget {
  const VerifyOrders({Key? key}) : super(key: key);

  @override
  State<VerifyOrders> createState() => _VerifyOrdersState();
}

class _VerifyOrdersState extends State<VerifyOrders> {
  List<dynamic> orders = []; 
  ...
  Future<void> fetchOrders() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.1.143:3000/DELIVERED_ORDERS'));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
        title: Text('Verify Order', style: TextStyle(color: Colors.white)),
                  ...
                  return GestureDetector(
                    onTap: () {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => VerifyOrdersDetails(
